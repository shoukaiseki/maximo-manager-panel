  ...
  <section class="query-page">
    <el-card>
      <div class="page-header-row">
        <div>
          <h2>操作管理</h2>
          <p class="page-summary">查询 ACTION 操作定义（改状态/设值/自定义类/操作组等），供工作流出线引用。以 "=" 开头精确匹配，支持 % 通配符模糊搜索。</p>
        </div>
        <div class="page-actions">
          <saved-query-panel ref="savedQuery" appname="ACTION" :default-where="buildWhere()" @whereChange="handleWhereChange" />
      ...
      <el-form :model="formData" ref="queryForm" :inline="true" label-width="70px" @submit.native.prevent>
        <el-form-item label="操作">
          <el-input v-model="formData.action" placeholder="=ACT_XXX 或 %STATUS%" clearable style="width: 200px;" @keyup.enter.native="handleQuery" />
        </el-form-item>
        <el-form-item label="类型">
          <el-select v-model="formData.type" placeholder="全部" clearable filterable style="width: 160px;" @change="handleQuery">
            <el-option v-for="t in typeOptions" :key="t.value" :label="t.label" :value="t.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="对象">
          <el-input v-model="formData.objectName" placeholder="=WORKORDER 或 %WO%" clearable style="width: 170px;" @keyup.enter.native="handleQuery" />
        </el-form-item>
        <el-form-item label="描述">
          <el-input v-model="formData.description" placeholder="输入描述关键词..." clearable style="width: 180px;" @keyup.enter.native="handleQuery" />
        </el-form-item>
        <el-form-item>
    ...
    <!-- 操作详情弹窗 -->
    <el-dialog :title="'操作详情 - ' + (currentRow ? currentRow.action : '')" :visible.sync="dialogVisible" width="860px" top="5vh" :close-on-click-modal="true">
          ...
          <div v-loading="detailLoading">
            <template v-if="detail">
              <el-descriptions :column="2" border size="small">
                ...
                <el-descriptions-item label="适用范围">{{ detail.useWith || '-' }}</el-descriptions-item>
                ...
                <el-descriptions-item v-if="detail.type === 'CHANGESTATUS'" label="备注" :span="2">{{ detail.memo || '-' }}</el-descriptions-item>
                <el-descriptions-item label="发送方系统" :span="2">{{ detail.senderSysId || '-' }}</el-descriptions-item>
              </el-descriptions>

              <div v-if="detail.actionGroup && detail.actionGroup.length">
                <p class="sub-title">操作组成员（{{ detail.actionGroup.length }}）</p>
                <el-table :data="detail.actionGroup" border stripe size="mini">
                  <el-table-column prop="sequence" label="顺序" width="80" />
                  <el-table-column prop="member" label="成员操作" min-width="220" show-overflow-tooltip />
                </el-table>
              </div>
            </template>
    ...
    <!-- 导出结果弹窗 -->
    <el-dialog title="导出操作" :visible.sync="exportDialog.visible" width="1000px" top="3vh" :close-on-click-modal="true" @opened="onExportDialogOpened">
      ...
      <div class="json-toolbar">
        <span style="float:left;color:#606266;line-height:32px;">{{ currentExportTab.label }}：共导出 {{ currentExportTab.total }} 个操作</span>
    ...
    <!-- 导入 JSON 弹窗 -->
    <el-dialog title="导入操作" :visible.sync="importDialog.visible" width="820px" top="3vh" :close-on-click-modal="false" @opened="onImportDialogOpened">
      <p style="margin:0 0 8px;color:#909399;font-size:12px;">粘贴 JSON（数组 或 {"actions": [...]}），支持导出结果直接导入；按 action 名称匹配更新或创建。</p>
...
import { actionList, actionDetail, actionExport, actionImport } from '@/api/wfdesign'
...
export default {
  ...
  data() {
    return {
      ...
      typeOptions: [
        { value: 'CHANGESTATUS', label: '更改状态' },
        { value: 'SETVALUE', label: '设置值' },
        { value: 'APPACTION', label: '应用操作' },
        { value: 'GROUP', label: '操作组' },
        { value: 'CUSTOM', label: '自定义类' },
        { value: 'EXECUTABLE', label: '命令行程序' },
        { value: 'ACCEPTINFERENCE', label: '接受推理' },
        { value: 'GETINFERENCE', label: '获取推理' }
      ],
      currentRow: null,
      detail: null,
      dialogVisible: false,
      detailTab: 'basic',
      // 详情导出 JSON: single=单条导出(detail 原样包装); cond=按当前查询条件全量导出
      detailJsonMode: 'single',
      detailJson: '',
      detailJsonSingle: '',
      detailJsonCond: '',
      detailEditor: null,
      // 导出: 两个参数组合 tab(当前条件 / 全部)
      exportDialog: {
        visible: false,
        loading: false,
        tab: 'cond',
        tabs: [
          ...
          { name: 'all', label: '全部操作', json: '', total: 0 }
        ]
      },
      exportEditor: null,
      // 导入
  ...
  methods: {
    initMainTableParam() {
      return {
        ownerName: 'action',
        uniqueId: 'action-list',
        sksAppName: 'maxaction',
        ...
        tableColumnList:
          this.sksUtils.newTableColumnList([
            ...
            { prop: 'objectName', label: '对象', width: 150 },
    ...
    buildWhere() {
      ...
      if (this.formData.action) {
        conds.push(this.likeCond('ACTION', this.formData.action))
      }
    ...
    resetForm() {
      this.formData = { action: '', type: '', objectName: '', description: '' }
    ...
    fillExportTab(name, data) {
      ...
      tab.total = (data && (data.actions || []).length) || 0
      tab.json = JSON.stringify(data || { actions: [] }, null, 2)
    },
    ...
    submitImport() {
      ...
      // 支持数组 / 导出格式 {actions:[...]} / 单个对象, 统一归一成纯数据包 {actions:[...]}
      let importData
      if (Array.isArray(parsed)) {
        importData = { actions: parsed }
      ...
      d.summary = null
      d.result = []
      actionImport(importData, { _impMode: d.impMode }).then(res => {
        const data = res.data || res
        if (data.status === 'error') {
    ...
    handleRowClick(row) {
      ...
      // 并行获取: 单条完整导出(detail) + 按当前查询条件批量导出
      const where = this.getEffectiveWhere()
      const condBody = where && where !== '1=1' ? { where: where } : {}
      Promise.all([
        actionDetail({ id: row.id }),
        actionExport(condBody)
        ...
        if (detailData && detailData.status === 'error') {
          ...
          this.detail = (detailData.actions || [])[0] || null
          // 单条导出 JSON 直接用 detail 响应(已包装成 {actions:[...]}, 可直接导入)
          this.detailJsonSingle = JSON.stringify(detailData || { actions: [] }, null, 2)
        }
        if (condData && condData.status === 'error') {
          this.$message.error('获取条件导出失败: ' + (condData.message || '未知错误'))
        } else {
          this.detailJsonCond = JSON.stringify(condData || { actions: [] }, null, 2)
        }
